...
export default function MyPostsScreen() {
  ...
  useEffect(() => {
    checkLoginAndLoadMyPosts();
  }, []);

  const checkLoginAndLoadMyPosts = async () => {
  ...
  const loadMyPosts = async () => {
    try {
      ...
      const response = await postApi.getMyPosts(0, 20, token || undefined);
      
      if (response.data?.success && response.data?.data?.posts) {
        console.log('내 작성글 로딩 성공:', response.data.data);
        setPosts(response.data.data.posts);
      } else {
        console.log('내 작성글 데이터 없음:', response.data);
        setPosts([]);
      }
    } catch (error: any) {
      console.error('내 작성글 로딩 오류:', error);
      console.error('응답 데이터:', error.response?.data);
      setErrorMessage('내 작성글을 불러오는데 실패했습니다.');
      setPosts([]);
    } finally {
      setLoading(false);
    }
    ...
    <ThemedView style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={[styles.header, {
          ...
          <ThemedText style={styles.headerTitle}>내 작성글</ThemedText>
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <ThemedText style={styles.loadingText}>내 작성글을 불러오는 중...</ThemedText>
          </View>
        ) : (
          <ScrollView
          ...
          >
            {posts.length === 0 ? (
              <View style={styles.emptyContainer}>
                ...
                />
                <ThemedText style={styles.emptyText}>아직 작성한 글이 없습니다</ThemedText>
                <TouchableOpacity
                  style={[styles.writeButton, {
                    backgroundColor: '#007AFF'
                  }]}
                  onPress={() => router.push('/community/write' as any)}
                >
                  <ThemedText style={styles.writeButtonText}>첫 글 작성하기</ThemedText>
                </TouchableOpacity>
              </View>
            ) : (
