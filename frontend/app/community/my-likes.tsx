...
export default function MyLikesScreen() {
  ...
  useEffect(() => {
    checkLoginAndLoadMyLikes();
  }, []);

  const checkLoginAndLoadMyLikes = async () => {
    const isLoggedIn = await requireLogin('내 좋아요 보기', showToast);
    if (isLoggedIn) {
      loadMyLikes();
    } else {
  ...
  const loadMyLikes = async () => {
    try {
      ...
      const response = await commentApi.getMyLikes(token, 0, 20);
      
             if (response.data?.success && response.data?.data?.posts) {
         console.log('내 좋아요 로딩 성공:', response.data.data);
         // 유효한 게시글만 필터링
         const validPosts = response.data.data.posts.filter((post: any) => post && post.id);
         setLikedPosts(validPosts);
       } else if (response.data?.success && response.data?.data?.content) {
         // 다른 응답 구조일 경우
         console.log('내 좋아요 로딩 성공 (content):', response.data.data);
         const validPosts = response.data.data.content.filter((post: any) => post && post.id);
         setLikedPosts(validPosts);
       } else if (response.data?.success && Array.isArray(response.data?.data)) {
         // 배열 형태로 직접 반환되는 경우
         console.log('내 좋아요 로딩 성공 (array):', response.data.data);
         const validPosts = response.data.data.filter((post: any) => post && post.id);
         setLikedPosts(validPosts);
       } else {
         console.log('내 좋아요 데이터 없음:', response.data);
         setLikedPosts([]);
       }
    } catch (error: any) {
      console.error('내 좋아요 로딩 오류:', error);
      console.error('응답 데이터:', error.response?.data);
      setErrorMessage('내 좋아요 목록을 불러오는데 실패했습니다.');
      setLikedPosts([]);
    } finally {
      setLoading(false);
    }
    ...
    <ThemedView style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={[styles.header, {
          ...
          <ThemedText style={styles.headerTitle}>내 좋아요</ThemedText>
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <ThemedText style={styles.loadingText}>내 좋아요를 불러오는 중...</ThemedText>
          </View>
        ) : (
          <ScrollView
          ...
          >
            {likedPosts.length === 0 ? (
              <View style={styles.emptyContainer}>
                ...
                />
                <ThemedText style={styles.emptyText}>아직 좋아요한 게시글이 없습니다</ThemedText>
                <TouchableOpacity
